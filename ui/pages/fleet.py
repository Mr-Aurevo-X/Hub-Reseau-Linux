...
def build(win: Any) -> Gtk.Widget:
    ...
    win._fleet_list = Gtk.ListBox()
    win._fleet_list.set_selection_mode(Gtk.SelectionMode.SINGLE)
    win._fleet_list.add_css_class("boxed-list")
    win._fleet_list.connect("row-selected", lambda _lb, row: _on_select(win, row))
    ...
    clamp.set_child(win._fleet_list)
...
def _on_select(win: Any, row: Gtk.ListBoxRow | None) -> None:
    if row is None:
        return
    win._fleet_selected_id = str(row.get_name() or "")
...
def render(win: Any) -> None:
    ...
    while True:
        row = win._fleet_list.get_row_at_index(0)
        if row is None:
            break
        win._fleet_list.remove(row)
    machines = list(win._fleet_store.get("machines") or [])
    if not machines:
        empty = Adw.ActionRow()
        empty.set_title(i18n.t("fleet_empty"))
        empty.set_activatable(False)
        win._fleet_list.append(empty)
        return
    restore = None
    for item in machines:
        row = Adw.ActionRow()
        row.set_name(str(item.get("id") or ""))
        row.set_title(str(item.get("name") or "?"))
        row.set_subtitle(
            f"{item.get('os') or '—'} · {item.get('address') or '—'} · "
            f"{item.get('location') or ''} · {_status_label(item)} · "
            f"{item.get('last_probe_at') or '—'} · {item.get('note') or ''}".replace(" ·  ·", " ·").rstrip(" ·")
        )
        badge = Gtk.Label(label=_status_label(item))
        badge.set_valign(Gtk.Align.CENTER)
        if item.get("last_online") is True:
            badge.add_css_class("success")
        elif item.get("last_online") is False:
            badge.add_css_class("error")
        row.add_suffix(badge)
        win._fleet_list.append(row)
        if item.get("id") == selected:
            restore = row
    if restore is not None:
        win._fleet_list.select_row(restore)
